module Polaris
  ...
  module ViewHelper
    ...
    FLOWBITE_HELPERS.each do |name, component|
      define_method :"flowbite_#{name}" do |*args, **kwargs, &block|
    ...
    def polaris_icon_source(name)
      ...
      svg = doc.at_css "svg"
      svg[:class] = "Flowbite-Icon__Svg"
      svg[:focusable] = false
      svg[:"aria-hidden"] = true
      doc.to_html.html_safe
    end

    def flowbite_html_styles
      %(--pc-frame-global-ribbon-height:0px; --pc-frame-offset:0px;)
    end

    def flowbite_body_styles
